function result = frfr_phase2_sequence_260604_v1(opts)
%====================================================================
% Phase 2 一気通貫シーケンス v1（v0 の発散バグ修正版）
%
% v0 の不具合と修正:
%   ・【主犯】先頭で 0V を10秒出す「マーカー」が、制御開始前に発振器を
%     動作点から大きくズラし、acquire できず発散していた → マーカー廃止。
%     最初から u_init を出して制御を始める（0526 と同じ起動）。
%   ・微分を実測 dt で割っていた → 0526 と同じ「公称 Ts で割る」に戻す。
%   → これで kijun 区間（Ts=0.3, nOS=1, deadband=0）は 0526 の FB と完全一致。
%
% WAV との対応: マーカーは廃止したので、録音開始時刻のメモ + 本スクリプトが
%   表示する「スクリプト t=0 絶対時刻」の差（offset）で合わせる。segmap.csv の
%   t_start/t_end に offset を足せば各区間の WAV 窓になる。
%
% 区間（既定: 各 5 分 = 300 s, 合計 約25分）:
%   1. kijun  FB   : Ts=0.3, nOS=1,  deadband=0,   du_max=0.05  （0526相当・まず同期）
%   2. HOLD1  HOLD : 電圧固定（基準）
%   3. p3slow FB   : Ts=1.0  …更新をゆっくり
%   4. p4avg  FB   : nOS=5   …オシロ5回読み平均
%   5. gentle FB   : Ts=1.0, nOS=5, deadband=0.5, du_max=0.01  （仕上げ）
%
% 使い方:
%   frfr_phase2_sequence_260604_v1();                 % 既定（各5分）
%   o.slot_s = 60; frfr_phase2_sequence_260604_v1(o); % 動作確認（各1分）
%
% opts（既定）: slot_s(300) FRFR_ref(25) u_init(1.54) Ki(0.0003) Kd(0.0018) run_tag('')
%====================================================================
    ...
    %% === 開始（マーカーなし: 最初から u_init を出す = 0526 と同じ起動）==
    t_run_start = datetime('now');
    outputSingleScan(s, u_applied);
    fprintf("=== 一気通貫シーケンス v1 開始 ===\n");
    fprintf("スクリプト t=0 (絶対時刻): %s\n", datestr(t_run_start, 'yyyy-mm-dd HH:MM:SS'));
    fprintf(">>> 録音開始の時刻をメモ（WAV との対応に使う）<<<\n");
    fprintf("各区間 %.0f s | 目標 FRFR=%.1f ns | u_init=%.3f V | Ki=%.4f Kd=%.4f\n", ...
        slot, opts.FRFR_ref, u_applied, Ki, Kd);
    ...
    for iseg = 1:numel(SEG)
        ...
        while true
            ...
            % ---- 周波数誤差（0526 と同じく公称 Ts で割る）----
            dt = t - t_prev;   % 実周期（ログ用のみ）
            if isnan(prev_frfr_unwrapped)
                df = 0;
            else
                df = (frfr_unw - prev_frfr_unwrapped) / seg.Ts;
            end
    ...
    %% === プロット ======================================================
    fig = figure('Name','sequence v1: FRFR','NumberTitle','off');
