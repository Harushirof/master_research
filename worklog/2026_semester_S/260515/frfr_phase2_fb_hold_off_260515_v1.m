function result = frfr_phase2_fb_hold_off_260515_v1(t_total, FRFR_ref, t_fb_end, t_hold_end)
%====================================================================
% Phase 2 コントローラ: FRFR を指定値に収束させる (ao0 単独構成)
% + 3段階シーケンス（FB → HOLD → OFF）
%
%   Stage FB   : 0          〜 t_fb_end     ・・・ 通常の FB 制御
%   Stage HOLD : t_fb_end   〜 t_hold_end   ・・・ FB 終端電圧で一定キープ
%   Stage OFF  : t_hold_end 〜 t_total      ・・・ 出力 0V（解放挙動を観察）
%
% デフォルト:
%   t_fb_end   =  300 s ( 5 分)
%   t_hold_end =  600 s (10 分: FB 5 分 + HOLD 5 分)
%   t_total    =  900 s (15 分: OFF 区間で 5 分の解放観察)
%
...
%====================================================================
    ...
    if nargin < 1 || isempty(t_total),    t_total    = 900; end
    ...
    fprintf("=== Phase 2 FB+HOLD+OFF 開始 (ao0 単独, 260515 v1) ===\n");
    ...
    fprintf("=== Phase 2 FB+HOLD+OFF 終了 ===\n");

    %% === CSV 保存 ======================================================
    timestamp = datestr(now, 'yyyymmdd_HHMMSS');
    log_name = sprintf('frfr_phase2_fb_hold_off_%s.csv', timestamp);
    ...
    title('Phase 2: FRFR (FB / HOLD / OFF)');
    exportgraphics(fig1, sprintf('frfr_phase2_hold_off_frfr_%s.pdf', timestamp), 'ContentType', 'vector');
    ...
    title('Phase 2: Phase Error (FB / HOLD / OFF)');
    exportgraphics(fig2, sprintf('frfr_phase2_hold_off_error_%s.pdf', timestamp), 'ContentType', 'vector');
    ...
    title('Phase 2: Control Voltage (FB / HOLD / OFF)');
    exportgraphics(fig3, sprintf('frfr_phase2_hold_off_ao0_%s.pdf', timestamp), 'ContentType', 'vector');
