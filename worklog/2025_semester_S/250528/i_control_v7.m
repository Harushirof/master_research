...
function realtime_ocxo_Icontrol_smoothed
    % --- NI DAQ 設定 ---
    d = daq("ni");
    addoutput(d, "Dev1", "ao0", "Voltage");  % 実機に合わせ

    % --- 制御パラメータ ---
    Ts = 1;                    % 制御周期 [秒]
    Ki = 1e-1;                    % 積分ゲイン [V/Hz/s]
    ...
    while ishandle(h1)
        ...
        % モニタ出力
        fprintf("t = %.1fs | Δf_avg = %+8.2f Hz | integral = %d | Vctrl = %.6f V\n", ...
            t_now, df_avg, integral, Vctrl);
