function main_v6()
    ...
    !ping 192.168.1.61
    % Siglent Technologies,SDS2204X HD,SDS2HBAD7R0419,2.5.1.2.2.5(初代No.2)
    % Siglent Technologies,SDS2204X HD,SDS2HBAD7R0418,2.5.1.2.2.5(2代目No.2)
    !ping 192.168.1.62
    ...
    write(triggerObj,'BURSt:INTernal:PERiod 8000');% burst periodを 8000sにする.

    %% WaveSurferを設定する. SDS2204Xを設定する.
    ts_ws1=datetime('now','Format','dd');
    ts_ws2=datetime('now','Format','M');
    ts_ws3=datetime('now','Format','yyyy,HH,mm,ss');
    ts_sds=datetime('now','Format','HHmmss');
    month_database=["JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP","OCT","NOV","DEC"];
    month_name=month_database(str2num(sprintf("%s",ts_ws2)));

    for i=1:N_WS4024HD
        wsObj(i).Timeout=30; % 30秒でタイムアウト
        write(wsObj(i),"STST ALL_DISPLAYED,HDD,FORMAT,BINARY");
        write(wsObj(i),sprintf("DATE %s,%s,%s",ts_ws1,month_name,ts_ws3));
    end
    for i=1:N_SDS2204XHD
    ...
    N_run=102; % 測定回数.
    % N_run=3; % 測定回数.
    t_save=6; % SDS2204XHDがUSBメモリに40MPt保存するのにかかる時間2024/01/18更新
    for run=1:N_run
