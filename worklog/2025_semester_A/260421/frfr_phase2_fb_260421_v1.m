function result = frfr_phase2_fb_260421_v1(t_total, FRFR_ref)
%====================================================================
% Phase 2 コントローラ: FRFR を指定値に収束させる
%
% Phase 1 の成功コード (run_frfr_fb_single.m) をベースに、
% 積分型更新に Ki * phase_error 項を追加。
%
% 制御則（積分型更新）:
...
%====================================================================
    ...
    SAFE_AO0 = 0.0;
    SAFE_AO1 = 0.0;
    c = onCleanup(@() cleanupDAQ(s, dev, SAFE_AO0, SAFE_AO1)); %#ok<NASGU>

    %% === 制御パラメータ ================================================
    Ts = 0.3;               % [s] サンプリング周期

    % 固定出力
    ao0_const = 1.54;       % [V] OCXO-A 用（固定）
    u_init    = 1.54;       % [V] ao1 初期値（平衡点付近）
    ...
    Ki = 0.0003;            % [V/ns]     位相誤差の積分ゲイン（Phase 2 で追加）
    Kd = 0.0018;            % [V/(ns/s)] 周波数誤差の減衰ゲイン（Phase 1 相当）
    ...
    fprintf("=== Phase 2 FB 開始 ===\n");
    fprintf("目標 FRFR = %.1f ns, 実験時間 = %.0f s\n", FRFR_ref, t_total);
    fprintf("Ki = %.4f, Kd = %.4f, Ts = %.1f s\n", Ki, Kd, Ts);
    ...
    while true
        ...
        if isnan(target_adjusted)
            % FRFR_ref(例:25ns) に最も近い unwrapped 値を計算
        ...
        % ---- 位相誤差 ----
        e_phase = target_adjusted - frfr_unwrapped;  % [ns]

        % ---- 周波数誤差 ----
        if isnan(prev_frfr_unwrapped)
            freq_err = 0;
        else
            freq_err = (frfr_unwrapped - prev_frfr_unwrapped) / Ts;  % [ns/s]
        end
        ...
        fprintf("t=%6.1f | FRFR=%.2f ns | e=%.2f ns | df=%.3f ns/s | du=%.5f | ao1=%.4f V\n", ...
    ...
    log_tbl = table( ...
        ...
        e_phase_log(:), freq_err_log(:), delta_u_log(:), ao1_log(:), ...
        'VariableNames', { ...
            'time_s', 'frfr_raw_ns', 'frfr_unwrapped_ns', ...
            'e_phase_ns', 'freq_err_ns_per_s', 'delta_u_V', 'ao1_V'});
    ...
    plot(time_log, ao1_log, 'LineWidth', 1.5);
    grid on; xlabel('Time [s]'); ylabel('ao1 [V]');
    title('Phase 2: Control Voltage');
    exportgraphics(fig3, sprintf('frfr_phase2_ao1_%s.pdf', timestamp), 'ContentType', 'vector');
    ...
    result.params = struct( ...
        ...
        'ao0_const', ao0_const, 'u_init', u_init, ...
...
function cleanupDAQ(s, dev, safe_ao0, safe_ao1)
    try, outputSingleScan(s, [safe_ao0, safe_ao1]); catch, end
